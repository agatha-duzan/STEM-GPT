#Code origin
#
#Author: Alexander Valentini (alexander.valentini@epfl.ch) and Nicholas (nicolas.almerge@epfl.ch)
#Made on: 26/5-2024
#Made for: Project report for course CS-552. submission on 2/6-2024
#Alexander has written code for those parts that are marked by comments (logprobs calculation).
...
#Alexander: Here we get the output dictionary from the model:
    ...
    def get_logprobs(self, batch, tokenizer):
        ...
        """
#Following code from Nicholas, here...        
#Alexander: Here we get the output dictionary from the model:
        ...
        for prompt, chosen, rejected in zip(prompts, chosen_responses, rejected_responses, strict=True):
            #Alexander: Here I took inspiration from the previous assignments - padding is set to the longest element
            ...
            #Alexander: We try to move the input to the device in order to speed up computations:
            #Alexnder: This has to be done by iterating through the dictionary - as this is the output from the tokenizer.
            ...
            #Alexander: Important to use torch.no_grad() to avoid memory issues (as we are evaluating):
            with torch.no_grad():
            ...
            #Alexander: Is is important to only evaluate based on the completions (responses) and not the prompt.
            ...
            #Alexander: Here I took inspiration with the gather function from Assignment 3:
        ...
        #Alexander: This concludes the logprob calculations.
        #Alexander: Following is contributed by Nicholas.
    ...
    def prediction_step_mcqa(self, batch, tokenizer):
        ...
        for example in batch:
            ...
            input_ids: torch.Tensor = tokenizer(question + "\n" + "\n".join(choices), return_tensors="pt", padding="longest", truncation=True).input_ids

            log_probs = F.log_softmax(self.forward(input_ids=input_ids).logits, dim=-1)
            prediction = torch.argmax(log_probs[:, -1, :], dim=-1).item()
            assert isinstance(prediction, int), "Prediction was expected to be an integer, assertion failed"

            # Make sure prediction is within bounds
            if prediction < 0: prediction = 0
            elif prediction >= len(choices): prediction = len(choices)-1

            # Add letter corresponding to prediction
