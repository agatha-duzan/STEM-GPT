...
class AutoDPOModelForCausalLM(PreTrainedModelWrapper):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # =============================================================
        output_dict = self.pretrained_model(input_ids=input_ids, past_key_values=past_key_values, attention_mask=attention_mask, **kwargs)
        ###############################################################
    ...
    def get_logprobs(self, batch, tokenizer):
        ...
        for prompt, chosen, rejected in zip(prompts, chosen_responses, rejected_responses):
            ...
            # debug: move to GPU if available
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            chosen_input = {key: val.to(device) for key, val in chosen_input.items()}
            rejected_input = {key: val.to(device) for key, val in rejected_input.items()}

            with torch.no_grad(): # no grad because inference
            ...
            len_prompt = len(prompt_token_ids)

            # seen in ED post: shift the logits by one to the left to align with next token
            chosen_log_probs = chosen_log_probs[0, len_prompt-1:len_prompt-1 + len(chosen_input["input_ids"][0]), :]
            rejected_log_probs = rejected_log_probs[0, len_prompt-1:len_prompt-1 + len(rejected_input["input_ids"][0]), :]

            chosen_token_ids = chosen_input["input_ids"][0][len_prompt:]
            rejected_token_ids = rejected_input["input_ids"][0][len_prompt:]
    ...
    def prediction_step_reward(
        ...
        beta=0.1 # value from original paper, used the same in my DPO
        output_dict = {
    ...
    def prediction_step_mcqa(self, batch, tokenizer):
        ...
        base_name = self.pretrained_model.name_or_path

        questions = batch["question"]
        preds = []

        for question in questions:
            if 'mcqa' in base_name:
                ...
                detailed_answer = tokenizer.decode(outputs[0], skip_special_tokens=True)

                # append prompt for prediction
                detailed_answer_with_prompt = detailed_answer + " Therefore, the correct answer is "

                # predict next token
                input_ids = tokenizer.encode(detailed_answer_with_prompt, return_tensors="pt").to(device)
                with torch.no_grad():
                    logits = self.pretrained_model(input_ids).logits

                next_token_logits = logits[:, -1, :]
                next_token_probs = F.softmax(next_token_logits, dim=-1)

                # final answer is set to letter token with highest prob
                candidates = ['A', 'B', 'C', 'D']
                candidates_ids = tokenizer.convert_tokens_to_ids(candidates)
                candidate_probs = {candidate: next_token_probs[0, candidate_id].item() for candidate, candidate_id in zip(candidates, candidates_ids)}
                predicted_answer = max(candidate_probs, key=candidate_probs.get)

                preds.append(predicted_answer)

            elif 'sft' in base_name:
                inputs = tokenizer(question, return_tensors="pt", padding=True, truncation=True).to(device)
                # generate only 1 token for answer
                # the FT model should generate A/B/C/D
                outputs = self.pretrained_model.generate(**inputs, 
                                                                max_new_tokens = 1,
                                                                num_return_sequences=1, 
                                                                pad_token_id=tokenizer.eos_token_id)
                short_answer = tokenizer.decode(outputs[0], skip_special_tokens=True)
                predicted_answer = short_answer[-1]
                preds.append(predicted_answer)
