...
class DPOModelEvaluator():
    ...
    def scoring_reward_computation(self, test_dataloader: DataLoader):
        ...
        for idx, batch in enumerate(tqdm(test_dataloader)):
            try:
                reference_chosen_logps = batch["chosen_logps"].to(device)
                reference_rejected_logps = batch["rejected_logps"].to(device)
                logger.debug(f"reference_chosen_logps shape: {reference_chosen_logps.shape}, device: {reference_chosen_logps.device}")
                logger.debug(f"reference_rejected_logps shape: {reference_rejected_logps.shape}, device: {reference_rejected_logps.device}")

                policy_chosen_logps, policy_rejected_logps = policy_model.get_logprobs(
                    batch, self.policy_tokenizer)
                logger.debug(f"policy_chosen_logps shape: {policy_chosen_logps.shape}, device: {policy_chosen_logps.device}")
                logger.debug(f"policy_rejected_logps shape: {policy_rejected_logps.shape}, device: {policy_rejected_logps.device}")
            ...
            policy_chosen_rewards.extend(reward.cpu().numpy() for reward in rewards[0])
            policy_rejected_rewards.extend(reward.cpu().numpy() for reward in rewards[1])
...
if __name__ == '__main__':
    ...
    # print(f"Checking repository setup in the current directory: {os.getcwd()}")
    repository_check()
    ...
    if "reward" in eval_method:
        ...
        # Loop over the remaining evaluation methods # rag and quant
